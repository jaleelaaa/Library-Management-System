...
import { FiPlus, FiEdit2, FiTrash2, FiSearch, FiFilter, FiX, FiEye, FiBookOpen, FiUsers, FiCalendar } from 'react-icons/fi'
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Course Reserves</h1>
          <p className="text-gray-600 mt-1">Manage courses and reserve materials</p>
        </div>
        <button
                  ...
                  {courses.map((course) => (
                    <tr key={course.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">
                        ...
                        {course.end_date && (
                          <div className="text-xs text-gray-500">to {formatDate(course.end_date)}</div>
                        )}
                      </td>
            ...
            {meta && meta.total_pages > 1 && (
              <div className="flex items-center justify-between px-6 py-4 border-t">
                <div className="text-sm text-gray-700">
                  Showing page {meta.page} of {meta.total_pages} ({meta.total_items} total courses)
                </div>
                <div className="flex gap-2">
                  <button
            ...
            {modalMode === 'view' && selectedCourse ? (
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Course Reserves ({reserves.length})</label>
                    {reserves.length === 0 ? (
                      <p className="text-gray-500 text-sm">No reserves added yet</p>
                    ) : (
                      <div className="border rounded-md">
                        {reserves.map((reserve, idx) => (
                          <div key={idx} className="p-3 border-b last:border-b-0">
                            <p className="text-sm text-gray-700">Item ID: {reserve.item_id}</p>
                            {reserve.reserve_type && (
                              <p className="text-xs text-gray-500">Type: {reserve.reserve_type}</p>
                            )}
                            {reserve.loan_period && (
                              <p className="text-xs text-gray-500">Loan Period: {reserve.loan_period}</p>
                            )}
                          </div>
                        ))}
                      </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t">
                  ...
                  >
                    {loading ? 'Saving...' : modalMode === 'create' ? 'Create Course' : 'Update Course'}
                  </button>
                </div>
              </form>
            )}
